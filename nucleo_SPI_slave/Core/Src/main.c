...
//	HAL_UART_Transmit(&huart2, (uint8_t*) user_data, len_of_data,
//	HAL_MAX_DELAY);
	...
	while (1) {
		...
		receiveStatus = HAL_SPI_Receive(&hspi2, rxBuffer, sizeof(rxBuffer),
				HAL_MAX_DELAY);
		...
		if (receiveStatus == HAL_OK) {
			...
			HAL_UART_Transmit(&huart2, rxBuffer, sizeof(rxBuffer),
					HAL_MAX_DELAY);
		} else {
		...
		HAL_Delay(1000); // Delay for a second before next receive attempt, adjust as needed
		/* USER CODE END WHILE */
...
static void MX_SPI2_Init(void) {
	...
	// As a slave, SPI2 doesn't generate a clock signal; it relies on the clock provided by the master
