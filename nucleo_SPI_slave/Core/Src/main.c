...
uint8_t rxBuffer[20]; // Buffer to store received data, adjust size as needed
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		receiveStatus = HAL_SPI_Receive(&hspi2, rxBuffer, 5,
				HAL_MAX_DELAY);

//		uint16_t len_of_data = strlen(receiving_data);
//		HAL_UART_Transmit(&huart2, (uint8_t*) receiving_data, len_of_data,
//				HAL_MAX_DELAY);
		...
		if (receiveStatus == HAL_OK) {
			...
			HAL_UART_Transmit(&huart2, rxBuffer, 5,
			HAL_MAX_DELAY);
		} else {
